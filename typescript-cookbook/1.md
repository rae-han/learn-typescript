...
> **팁**: `// @ts-ignore`보다 `// @ts-expect-error`를 권장한다. `@ts-ignore`는 오류 해결 후에도 주석이 남아 무시되지만, `@ts-expect-error`는 오류가 사라지면 "예상한 오류가 발생하지 않음" 경고를 띄워 정리 시점을 알려준다.
...
> **2025 업데이트**: TypeScript 5.x에서는 `target: "es2022"`가 일반적인 권장 값이다. `es2022`는 `Array.prototype.at()`, top-level await, `#private` 필드 등을 포함하며 Node.js 18+와 최신 브라우저에서 지원된다.
...
- 각 파일을 `target`(여기서는 `es2022`)에 맞춰 다운레벨링하고, `outDir`에 지정된 폴더(`dist`)로 JavaScript 결과물을 출력한다.
- `strict: true` 설정이 있다면 모든 타입 검사를 엄격하게 수행하고, 오류가 발생하면 종료 코드가 1로 설정되어 CI에서도 쉽게 감지할 수 있다.

> **팁**: 프로젝트 초기화 시 `npx tsc --init`이 생성하는 `tsconfig.json`에는 모든 옵션이 주석으로 설명되어 있다. 필요한 옵션만 주석 해제하며 점진적으로 설정을 확장하는 것이 좋다.
...
> **팁**: JSDoc `@typedef`와 `import()` 구문을 조합하면 `.d.ts` 타입을 JS 파일에서도 재사용할 수 있다. 대규모 JS 코드베이스를 점진적으로 TS로 전환할 때 유용한 패턴이다.
...
> **2025 업데이트**: TypeScript 5.x의 `satisfies` 연산자를 활용하면 타입 단언(`as`) 없이도 값의 타입 호환성을 검사할 수 있다. 마이그레이션 중 `any`를 줄이고 타입 안전성을 높이는 데 유용하다.
...
> **팁**: 최근 많은 라이브러리가 자체적으로 타입을 번들링하여 배포한다(예: axios, zod). `@types/*` 설치 전에 라이브러리가 이미 타입을 포함하는지 확인하자. `package.json`의 `types` 또는 `typings` 필드가 있으면 별도 설치가 필요 없다.
...
> **2025 업데이트**: TypeScript 5.x에서 추가된 `moduleResolution: "bundler"` 옵션은 Vite, esbuild 등 번들러 환경에 최적화되어 있다. ESM과 CJS 혼용 시 발생하는 해석 문제를 줄여주며, 풀스택 프로젝트에서 권장된다.
...
> **팁**: 모노레포 환경에서는 Turborepo나 Nx 같은 빌드 시스템과 TypeScript 프로젝트 참조를 함께 사용하면 캐싱과 병렬 빌드 이점을 누릴 수 있다.
...
> **2025 업데이트**: Vitest가 Jest의 대안으로 빠르게 성장하고 있다. Vitest는 ESM을 네이티브로 지원하고, Vite 기반 프로젝트와 쉽게 통합된다. TypeScript 설정 없이도 `.ts` 파일을 바로 테스트할 수 있어 설정이 간소화된다.
...
> **팁**: `tsx`(esbuild 기반)를 사용하면 `ts-node`보다 훨씬 빠른 트랜스파일 속도를 얻을 수 있다. `npx tsx watch src/index.ts` 형태로 개발 서버 실행에도 활용 가능하다.
...
> **팁**: Import Maps(`<script type="importmap">`)를 활용하면 브라우저에서도 bare specifier(`import _ from "lodash"`)를 사용할 수 있다. TypeScript의 `paths`와 함께 설정하면 개발과 프로덕션 환경에서 일관된 import 경로를 유지할 수 있다.
...
import type { Person } from "./person.mjs";
const { printPerson } = require("./person.cjs");

printPerson({ name: "Stefan", age: 40 } satisfies Person);
```

> **2025 업데이트**: Node.js 20+에서는 ESM이 기본으로 안정화되었고, `--experimental-require-module` 플래그로 `require()`에서 ESM 모듈을 로드할 수 있다. TypeScript 5.x의 `verbatimModuleSyntax` 옵션을 켜면 import/export 문법이 모듈 시스템에 맞게 정확히 출력된다.
...
> **팁**: 라이브러리를 배포할 때는 `tsup`이나 `unbuild` 같은 도구를 사용하면 ESM/CJS 듀얼 패키지를 쉽게 생성할 수 있다. 수동으로 `.mts`/`.cts` 파일을 관리하는 것보다 빌드 도구에 위임하는 것이 유지보수에 유리하다.
...
> **2025 업데이트**: Deno 2.0에서는 `package.json`과 `node_modules`를 네이티브로 지원하여 Node.js 프로젝트를 거의 수정 없이 실행할 수 있다. `deno install`로 npm 패키지를 설치하고, 기존 Node.js 생태계와의 호환성이 크게 향상되었다.
...
> **팁**: JSR(JavaScript Registry)은 Deno 팀이 만든 새로운 패키지 레지스트리로, TypeScript를 네이티브로 지원한다. `jsr:@std/path` 같은 형태로 import하면 타입 정보가 자동으로 제공된다.
...
`@tsconfig/*` 패키지를 설치해 `tsconfig.json`의 `extends`에 연결하면 곧바로 권장 옵션을 사용할 수 있다. 예를 들어 Node.js 20 + ESM + Strict 환경은 다음과 같이 적용한다.
...
> **2025 업데이트**: TypeScript 5.x에서는 `@tsconfig/strictest` 프리셋이 인기를 얻고 있다. `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes` 등 가장 엄격한 검사 옵션을 모두 활성화하여 타입 안전성을 극대화한다.
...
> **팁**: 프레임워크별 공식 프리셋을 우선 확인하자. Next.js는 `next.config.js`에서 자동으로 `tsconfig.json`을 생성하고, Remix나 SvelteKit도 초기 설정 시 최적화된 TypeScript 설정을 제공한다.
