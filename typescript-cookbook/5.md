...
> **팁**: 이 패턴을 활용하면 "대상 객체에서 메서드 이름과 인수를 추론해 호출하는" API를 깔끔하게 설계할 수 있다. 프록시 패턴이나 RPC 래퍼에서 특히 유용하다.
...
> **팁**: 이 원리를 알면 필요한 필터를 직접 만들 수도 있다. 예를 들어 숫자 인덱스를 제외한 메서드만 남기고 싶다면 `Exclude<keyof ElementList, number>`로 충분하다. 더 복잡한 조건이 필요하면 `T[K] extends (...args: any) => any ? K : never` 같은 매핑 타입을 결합하면 된다.

| 유틸리티 | 동작 | 구현 |
|---------|------|------|
| `Exclude<T, U>` | U에 할당 가능한 멤버 제거 | `T extends U ? never : T` |
| `Extract<T, U>` | U에 할당 가능한 멤버만 유지 | `T extends U ? T : never` |
| `NonNullable<T>` | null과 undefined 제거 | `T & {}` (TS 5.0+) |
...
> **팁**: 이 방식은 상태별로 데이터를 모으거나, REST 응답을 타입 안전하게 분류할 때도 동일하게 적용할 수 있다.

> **2025 업데이트**: TypeScript 5.0+에서 `satisfies`와 함께 사용하면 그룹핑 결과의 타입을 더 정확하게 검증할 수 있다:
> ```ts
> const grouped = groupToys(toys) satisfies GroupedToys;
> // 타입 체크와 함께 추론된 타입 유지
> ```
...
> **팁**: 반대로 특정 타입만 추출하고 싶다면 다음 패턴을 사용한다:
> ```ts
> type PickByType<T, U> = {
>   [K in keyof T as T[K] extends U ? K : never]: T[K];
> };
> type StringProps = PickByType<Person, string>;
> // { name: string; profession?: string | undefined }
> ```

> **2025 업데이트**: TypeScript 5.4의 `NoInfer`를 조건부 타입과 함께 사용하면 더 정밀한 타입 필터링이 가능하다:
> ```ts
> type FilterKeys<T, U> = {
>   [K in keyof T as T[K] extends NoInfer<U> ? K : never]: T[K];
> };
> ```
...
> **팁**: 반환 시 `as Serialize<T>` 어서션이 필요한 이유는 타입스크립트가 런타임 분기를 충분히 이해하지 못하기 때문이다. 구현과 타입 정의가 동기화되어 있다면 안전한 어서션으로 볼 수 있다.

> **노트**: 조건부 타입과 재귀 매핑 타입을 조합하면 직렬화/검증 같은 반복 패턴을 타입 수준에서 선제적으로 모델링할 수 있다. JSON.stringify의 결과 타입을 정확히 표현할 때도 이 패턴을 활용한다.
...
| 패턴 | 용도 | 예시 |
|-----|------|------|
| 분배 조건부 타입 | 유니온 각 멤버에 적용 | `T extends U ? X : Y` |
| infer 키워드 | 타입 추출 | `T extends (infer R)[] ? R : never` |
| 재귀 조건부 타입 | 중첩 구조 처리 | `Serialize<T>` |
| never 필터링 | 멤버 제거 | `T extends U ? never : T` |
| as 절 재매핑 | 키 변환/제거 | `[K in keyof T as ...]` |

## infer 활용 패턴

| 유틸리티 | 추출 대상 | 구현 |
|---------|----------|------|
| `ReturnType<T>` | 함수 반환 타입 | `T extends (...args: any) => infer R ? R : never` |
| `Parameters<T>` | 함수 매개변수 튜플 | `T extends (...args: infer P) => any ? P : never` |
| `Awaited<T>` | Promise 내부 타입 | `T extends Promise<infer U> ? Awaited<U> : T` |
| `InstanceType<T>` | 클래스 인스턴스 타입 | `T extends new (...args: any) => infer R ? R : never` |
