...
“원시형 제외”가 목적이면 `object`, “키가 문자열이어야 함”이 목적이면 `Record<string, unknown>`처럼 의도를 드러내는 타입을 선택하자.
...
- **의미 있는 위치 이름 붙이기**: 타입 별칭 내에서 주석을 달거나 JSDoc을 활용해 `[위도, 경도]` 같은 의미를 명시하면 유지보수가 쉬워진다.
...
- 타입스크립트는 `noImplicitThis` 옵션을 통해 `this` 타입이 추론되지 않는 상황을 경고할 수 있다. 프로젝트 단위로 켜 두면 실수로 `any`가 퍼지는 것을 막을 수 있다.
