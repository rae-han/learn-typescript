...
> **팁**: 유니온 타입이 3개 이상이면 구별된 유니온(3.2절)을 고려하자. `kind` 같은 판별자 속성을 추가하면 `switch` 문으로 깔끔하게 분기할 수 있다.
...
> **2025 업데이트**: TypeScript 5.x에서는 구별된 유니온의 타입 좁히기가 더욱 강화되었다. 특히 중첩된 객체의 판별자도 인식하고, `switch`의 `default` 분기에서 남은 타입을 정확히 추론한다.

> **팁**: Redux 액션, API 응답, 이벤트 타입 등 "케이스별로 구분되는 데이터"에서 구별된 유니온은 필수 패턴이다. `type Action = { type: "ADD"; payload: Item } | { type: "REMOVE"; id: string }` 형태가 대표적이다.
...
> **팁**: `assertNever`는 프로젝트 공통 유틸리티로 두고 재사용하자. `ts-essentials` 같은 라이브러리에도 동일한 헬퍼가 포함되어 있다.
...
> **2025 업데이트**: `satisfies`와 `as const`를 함께 사용하면 타입 검증과 리터럴 보존을 동시에 달성할 수 있다. 이 조합은 설정 객체나 테마 정의에서 특히 유용하다.
...
// theme.primary는 "#007bff" 리터럴 타입
```

> **팁**: `as const`는 중첩된 객체에도 재귀적으로 적용된다. 깊은 객체 구조에서도 모든 프로퍼티가 리터럴 타입으로 고정된다.
...
> **2025 업데이트**: Zod, Valibot 같은 런타임 검증 라이브러리를 사용하면 타입 가드를 직접 작성하지 않아도 된다. 스키마 정의에서 타입 가드 함수가 자동으로 생성된다.

```ts
import { z } from 'zod';
const ApiErrorSchema = z.object({ status: z.number(), message: z.string() });
// ApiErrorSchema.safeParse(value).success가 타입 가드 역할
```

> **팁**: 어서션 함수(`asserts value is Type`)를 사용하면 검증 실패 시 예외를 던지면서 타입도 좁힐 수 있다. 초기 검증 로직에서 유용하다.
...
| 타입 | 의미 | 반환 가능 값 | 호출자가 값 사용 |
|------|------|-------------|-----------------|
| `void` | 반환값 무시 | `undefined`, 없음 | ❌ |
| `undefined` | 명시적 undefined | `undefined` | ✅ |
| `never` | 반환하지 않음 | 없음 (throw/무한루프) | ❌ |
...
> **팁**: `--noImplicitReturns` 옵션과 함께 사용하면 모든 코드 경로에서 `return` 문이 존재하는지 확인할 수 있다. 의도치 않은 `undefined` 반환을 막고 싶을 때 유용하다.
...
> **2025 업데이트**: TypeScript 4.4부터 `strict: true` 설정에 `useUnknownInCatchVariables`가 포함되어 `catch` 변수가 기본적으로 `unknown` 타입이 된다. 이전 버전에서는 별도로 옵션을 활성화해야 한다.

ES2022부터 타입스크립트는 기본적으로 `catch` 변수의 타입을 `unknown`으로 간주한다. 안전하게 다루려면 다음과 같은 패턴을 활용한다.
...
> **팁**: `Error.cause` 속성(ES2022)을 활용하면 오류 체인을 구성할 수 있다. `new Error("래핑된 오류", { cause: originalError })`처럼 사용한다.
...
> **팁**: 3개 이상의 상호 배타적 옵션이 있다면 구별된 유니온(`kind` 속성 사용)이 더 명확할 수 있다. 2개일 때는 `never` 패턴이 간결하다.
...
> **2025 업데이트**: `satisfies` 연산자는 타입 호환성을 검사하면서 추론된 타입을 유지한다. `as`와 달리 타입을 강제로 변환하지 않으므로 더 안전한 대안이 될 수 있다.

> **팁**: 비널 어서션(`!`)은 값이 `null`/`undefined`가 아님을 단언한다. 남용하면 런타임 오류로 이어지므로, 확실한 경우에만 사용하자.
...
> **팁**: `noUncheckedIndexedAccess` 옵션을 켜면 인덱스 시그니처를 통해 접근할 때 `undefined` 가능성이 자동으로 포함된다. 런타임 존재 여부를 체크해야 하는 상황에서 유용하다.
...
> **2025 업데이트**: `exactOptionalPropertyTypes` 옵션(TypeScript 4.4+)을 켜면 `nickname?: string`은 "속성 없음 또는 string"만 허용하고, `undefined`를 명시적으로 할당하려면 `nickname?: string | undefined`로 선언해야 한다.
...
> **팁**: `Object.hasOwn(obj, "key")`는 `"key" in obj`보다 안전하다. 프로토타입 체인을 검사하지 않고 자체 속성만 확인한다.
...
| 방식 | 런타임 객체 | 트리쉐이킹 | 권장 상황 |
|------|-----------|-----------|----------|
| `enum` | ✅ | ❌ | 디버깅 필요, 역매핑 필요 |
| `const enum` | ❌ (인라인) | ✅ | 번들 크기 중요 |
| 리터럴 유니온 | ❌ | ✅ | 라이브러리, 간단한 상수 |
...
> **팁**: 많은 프로젝트에서 `enum` 대신 `as const` 객체 + 리터럴 유니온 패턴을 선호한다. 번들 크기와 트리쉐이킹 이점이 있고, JavaScript 생태계와의 호환성도 좋다.
...
> **2025 업데이트**: `ts-brand`, `effect`(Effect-TS) 같은 라이브러리에서 브랜딩 유틸리티를 제공한다. 직접 구현하는 대신 검증된 라이브러리를 사용하는 것도 좋은 선택이다.

> **팁**: 브랜딩은 데이터베이스 ID, 통화 단위, 측정 단위가 다른 수치값 등을 혼동 없이 처리하는 데 큰 도움이 된다. `type USD = Brand<"USD", number>`와 `type EUR = Brand<"EUR", number>`는 서로 호환되지 않는다.
...
> **팁**: `(typeof STATUSES)[number]` 패턴은 어떤 `as const` 배열에도 적용할 수 있다. 객체의 키 집합을 추출할 때는 `keyof typeof OBJ`, 값 집합은 `(typeof OBJ)[keyof typeof OBJ]`를 사용한다.
