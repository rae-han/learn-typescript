...
// 위 코드에서 response.data.* 를 해도 에러가 안뜨네?? any이기 때문...
// any로 도배를 한다면 typescript를 사용하는 이유가 없다.
// 데이터 분석 해보면 get메서드의 첫번째 제네릭이 response 타입 자리다.
...
(async () => {
  try {
    const response1 = await axios.get<Post>('https://jsonplaceholder.typicode.com/posts/1')
    ...
    const response3 = await axios.post<Created, AxiosResponse<Created>, Data>('https://jsonplaceholder.typicode.com/posts', {
      title: 'foo',
      body: 'bar',
      'userId': 1,
    })
...
(async () => {
  try {
    ...
    const errorResponse1 = (error as AxiosError).response
