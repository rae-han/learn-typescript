...
function add2<T extends number | string>(x: T, y: T): T { return x + y }
// 만약 위 코드가 에러가 난다면 T가 먼지 몰라서 T+T가 뭘 의미하는지 모른다.

// 제네릭을 여러개 만들면서 각각 다른 제한을 둘 수도 있다.
function add3<T extends number, K extends string>(x: T, y: K): T { return x + y }

// 제한 방법
// 첫번째는 바로 타입을 넣어버리는 것
function add4<T extends { a: number }>(x: T): T { return x }
function add5<T extends number[]>(x: T): T { return x }
add5([1, 2, 3]);
// 함수 모양으로 제한을 둘 수도 있다.
